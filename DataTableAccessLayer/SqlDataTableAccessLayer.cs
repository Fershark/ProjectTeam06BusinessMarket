...
namespace DataTableAccessLayer
{
    ...
    public class SqlDataTableAccessLayer
    {
        ...
        protected bool restoringFromBackup = false;
        ...
        public void InsertTableRow(DataRow row)
        {
            ...
            // It should also consider if we are restoring from a backup as data can be inserted
            // row by row.

            if (row == null || (!restoringFromBackup && row.RowState != DataRowState.Added))
                return;
